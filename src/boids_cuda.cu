...
__device__ CellCoords get_cell_cords(const SimulationParameters *sim_params, const glm::vec3& position) {
    return CellCoords {
...
__device__ CellId get_flat_cell_id(const SimulationParameters *sim_params, const glm::vec3& position) {
...
GPUBoids::GPUBoids(const boids::Boids& boids) {
    int deviceCount;
...
__global__ void find_cell_ids(const boids::SimulationParameters *params, BoidId *boid_id, CellId *cell_id, glm::vec3 *position, size_t boids_count) {
...
__global__ void update_acceleration(const SimulationParameters *params, BoidId *boid_id, glm::vec3 *position, glm::vec3 *velocity, glm::vec3 *acceleration, size_t boids_count) {
    BoidId b_id = blockIdx.x * blockDim.x + threadIdx.x;
    if (b_id >= boids_count) return;
...
//    }
    ...
    acceleration[b_id] =
            ...
            params->cohesion * (avg_pos - position[b_id]);
...
__global__ void update_simulation(const SimulationParameters *params, glm::vec3 *position, glm::vec3 *velocity, glm::vec3 *acceleration, glm::vec3 *forward, glm::vec3 *up, glm::vec3 *right, size_t boids_count, float dt) {
    BoidId b_id = blockIdx.x * blockDim.x + threadIdx.x;
    if (b_id >= boids_count) return;
    ...
    position[b_id] += velocity[b_id] * dt;

    // Update orientation
    forward[b_id] = glm::normalize(velocity[b_id]);
    right[b_id] = glm::normalize(glm::cross(up[b_id], forward[b_id]));
    up[b_id] = glm::normalize(glm::cross(forward[b_id] , right[b_id]));
}
...
void GPUBoids::update_simulation_with_sort(const boids::SimulationParameters &params, Boids &boids, float dt) {
    ...
    // 5.
    update_simulation<<<blocks_num, threads_per_block>>>(m_dev_sim_params, m_dev_position, m_dev_velocity, m_dev_acceleration, m_dev_forward, m_dev_up, m_dev_right, SimulationParameters::MAX_BOID_COUNT, dt);
    cudaDeviceSynchronize();
    ...
    check_cuda_error(cuda_status, "[CUDA]: cudaMemcpy failed: ");
    cuda_status = cudaMemcpy(boids.forward, m_dev_forward, arr_size, cudaMemcpyDeviceToHost);
    check_cuda_error(cuda_status, "[CUDA]: cudaMemcpy failed: ");
    cuda_status = cudaMemcpy(boids.up, m_dev_up, arr_size, cudaMemcpyDeviceToHost);
    check_cuda_error(cuda_status, "[CUDA]: cudaMemcpy failed: ");
    cuda_status = cudaMemcpy(boids.right, m_dev_right, arr_size, cudaMemcpyDeviceToHost);
    check_cuda_error(cuda_status, "[CUDA]: cudaMemcpy failed: ");
}
